...
//using PluginTemplate.Migrations.SeedData;
//using PluginTemplate.MultiTenancy;
//using PluginTemplate.Users;
...
{
    public abstract class PluginTemplateTestBase : AbpIntegratedTestBase<PluginTemplateTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PluginTemplateTestBase()
        {
        ...
        protected void UsingDbContext(Action<PluginTemplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PluginTemplateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PluginTemplateDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PluginTemplateDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PluginTemplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PluginTemplateDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PluginTemplateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PluginTemplateDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PluginTemplateDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PluginTemplateDbContext, Task<T>> func)
        {
            T result;
